...
class CCoilGapsZ_YZ(KlayoutPyGradCoil):
    ...
    def __init__(self, TopCellName, GradCoil):
        ...
        self.TargetCellName = "GZ{}_{}_red".format(self.zNOT_bot,
                                                   self.zNOT_top)
    ...
    def read_grad_streamlines(self, GradCoil):
        self.blue_cc01, self.red_cc01 = import_svg_polyline(
            self.dir_grad_coils + GradCoil["@fileCoil_bot"])
        self.blue_gaps_cc01, self.red_gaps_cc01 = \
            import_svg_polyline(self.dir_grad_coils
                                + GradCoil["@fileGaps_bot"])
        self.blue_cc00, self.red_cc00 = import_svg_polyline(
            self.dir_grad_coils + GradCoil["@fileCoil_top"])
        self.blue_gaps_cc00, self.red_gaps_cc00 = import_svg_polyline(
            self.dir_grad_coils + GradCoil["@fileGaps_top"])

    @staticmethod
    def KlayoutToBfield(cond, z_not):
        """
        return mapped array for/from Klayout to Bfield calculation
    ...
    def simuGradModel(self):
        ...
        for cond in self.blue_cc01:
            self.conductors.append([self.KlayoutToBfield(cond,
                                                         -1.0 * self.ccSep01),
                                    {"color": "b", "zorder": 210}])
    ...
    def makeInterconnect(self, InterConnectBaseArr, LayerInfo=None):
        ...
        inter_con_top = inter_con_top + InterConnectBaseArr \
            * np.array([1.0, -1.0])

        self.RoundPathF(inter_con_top, 300, 400, LayerInfo)
        self.RoundPathF(InterConnectBot[0:3], 300, 400)
        self.RoundPathF(InterConnectBot[0:2], 300, 400 + 60,
                        self.LayerSU8TopLitho)
    ...
    def conductorModelBcal(self):
        ...
        # zNot is in um
        zPos_cc01 = np.zeros_like(xPos_cc01) + (self.zNOT_bot * 1e-3)
        ...
        BotChipConductorSet = np.asarray([np.flipud(
            np.append(zPos_top, zPos_cc01)),
                                          np.flipud(
                                              np.append(xPos_top, xPos_cc01)),
                                          np.flipud(
                                              np.append(yPos_top, yPos_cc01))])

        self.conductors.append([np.hstack([InterconnectStart02_zNotPos,
                                           InterconnectStart01_zNotPos,
                                           BotChipConductorSet,
                                           InterconnectEnd_zNotPos]).swapaxes(
                                               0, 1),
                                {"current": 1.0}])
        ...
        self.condPlot3D.append([
            BotChipConductorSet.swapaxes(0, 1)[0:len(xPos_cc01)+1],
            {"color": "orangered", "alpha": 0.7, "zorder": 90}])

        self.condPlot3D.append([
            BotChipConductorSet.swapaxes(0, 1)[len(xPos_cc01)+1:-1],
        ...
        InterconnectStart01_zNotPos_Top = \
            (np.flipud(InterconnectStart01_zNotPos).T
             * np.array([1.0, -1.0, -1.0])).T
        InterconnectStart02_zNotPos_Top = \
            (np.flipud(InterconnectStart02_zNotPos).T
             * np.array([1.0, -1.0, -1.0])).T
        ...
        self.conductors.append([helperSS(TopChipComplementSet),
                                {"current": 1.0}])
        ...
        self.condPlot3D.append([
            helperSS(TopChipConductorSet)[len(xPos_cc01)+1:-1][::-1],
            {"color": "slateblue", "alpha": 1.0, "zorder": 199}])

        self.condPlot3D.append([
            helperSS(TopChipConductorSet)[0:len(xPos_cc01)+1][::-1],
            {"color": "mediumblue", "alpha": 1.0, "zorder": 200}])

        self.condPlot3D.append([
            helperSS(InterconnectStart01_zNotPos_Top)[::-1],
            {"color": "g", "alpha": 1.0, "zorder": 190}])

        self.condPlot3D.append([
            helperSS(InterconnectStart02_zNotPos_Top)[::-1],
            {"color": "g", "alpha": 1.0, "zorder": 190}])
...
def makeCoilGapsZ_YZ(GradCoil, TargetCellName,
    ...
    listOfCoils_cc01 = Grad.GapsIntoCoilList(
        Grad.red_cc01,
        ptsIntercon[:Grad.shiftEndIcon],
        interconDistMax=Grad.interconDistBot)
    listOfCoils_gaps_cc01 = Grad.GapsIntoCoilList(
        Grad.red_gaps_cc01,
        ptsIntercon[:Grad.shiftEndIcon],
        interconDistMax=Grad.interconDistBot)

    # make the coil curves
    SingleArray_cc01 = Grad.makeSingleCurve(listOfCoils_cc01,
                                            ptsIntercon[:Grad.shiftEndIcon])
    Grad.SingleArray_cc01 = np.append(SingleArray_cc01,
                                      ptsIntercon[Grad.shiftEndIcon:],
                                      axis=0)

    listOfCoils_top = Grad.GapsIntoCoilList(
        Grad.red_cc00,
        ptsIntercon[:Grad.shiftEndIcon],
        interconDistMax=Grad.interconDistTop)
    listOfCoils_gaps_top = Grad.GapsIntoCoilList(
        Grad.red_gaps_cc00,
        ptsIntercon[:Grad.shiftEndIcon],
        interconDistMax=Grad.interconDistTop)
    ...
    SingleArray_top = Grad.makeSingleCurve(listOfCoils_top,
                                           ptsIntercon[:Grad.shiftEndIcon])
    SingleArray_top = np.append(SingleArray_top,
                                ptsIntercon[Grad.shiftEndIcon:],
                                axis=0)
