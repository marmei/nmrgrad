...
class FluidicChannels(KlayoutPyFeature):
    ...
    def makeXYPointsFromRad(self, nPoints):
        ...
        returns np.array([[xPos1, yPos1], ...,  [xPosN, yPosN]]), [angle1, ..., angleN]
        """
        ...
        yPoint = (np.cos(angles) * radius - np.cos(angles[-1]) * radius + self.y1) * 1e3
    ...
    def makePrimers(self, sizeShape=0.0, primerTip=False):
        ...
        if primerTip == True:
            conPoly = np.array([[0.5 * width,
                                ...
                                [0.5 * width,
                                 + addScale - 0.5* self.addSpacerScale - self.baseSizeShapes],
                                [-0.5 * width,
                                 + addScale - 0.5* self.addSpacerScale - self.baseSizeShapes],
                                [-0.5 * width,
                                 -self.lenFconnector - addScale]])
        ...
        points_inlets, angles_inlet = self.makeXYPointsFromRad( self.nInlets + 2 )
        outlets = list()
        angleOutlets = list()
        ...
        for n in range(1, len(points_inlets+2)-1):
            correct_displ = (self.widthMain  * 0.5 - np.cos( angles_inlet[n] ) * self.widthMain  * 0.5)
            xICp = points_inlets[n][0]
            yICp = points_inlets[n][1]- correct_displ - self.widthMain  * 0.5
            TransformICP = pya.ICplxTrans().new(1.0,  # scale
                                                -1 * angles_inlet[n] / np.pi * 180, # angle
                                                False,
                                                xICp, # move x
                                                yICp) # move y
            polyShape = poly.round_corners(self.round_outlet, self.round_inlet, 32)

            if skip_outlet_counter not in self.skipOutlet and (self.SkipPrimerBool != True): # skip the outlets
                polyPlace = polyShape.transformed(TransformICP)
                self.insert(polyPlace)
                polyPlaceMirror = polyPlace.transform(pya.Trans(pya.Trans.M0, -0.0, -0.0))
                self.insert(polyPlaceMirror)
            outlets.append(np.array([xICp - np.sin(angles_inlet[n]) * (self.lenFconnector),
                                        yICp - np.cos(angles_inlet[n]) * (self.lenFconnector),
                                        angles_inlet[n],
                                        xICp - np.sin(angles_inlet[n]) * (self.lenFconnector) * 0.5,
                                        yICp - np.cos(angles_inlet[n]) * (self.lenFconnector) * 0.5]))
    ...
    def makeChannelsYXpos(self):
        """
        returns the evenly spaced channel xpositions for an internal defined number of inlets.
        """
    ...
    def makeChannelsY(self, channels, ypos, sizeShape):
        ...
        @channelPos: np.array([[channel_1_x1, ..., channel_n_x1], ..., [channel_1_xn, ..., channel_n_xn]])
        @ypos:       np.array([ y1, ...,  yn])
        """
        if self.outlets == None:
            raise Exception("run self.makePrimers first or define parameter outputs!")
        lenPoints = len(channels)
        for Idx in range(len(channels[0])):
            if np.absolute(self.outlets[Idx][2]) <= 0.01: # for a small angle, go straight
                channelPath = [self.outlets[Idx], [channels[0][Idx], self.channelYknick]]
            else:
                slopeChannelOut = np.tan( self.outlets[Idx][2])
            ...
            channel_poly = pya.Path( arrayToPointList(channelPath), widthChannel, 0.0, 0.0, True)
    ...
    def makeChannelsYSpline(self, channels, ypos, sizeShape = 0.0):
        ...
        :param channels: np.array([[channel_1_x1, ..., channel_n_x1], ..., [channel_1_xn, ..., channel_n_xn]])
        ...
        for Idx in range(len(channels[0])):
            ...
            outlet_02 = (self.outlets[Idx][0:2] \
                         + 0.5 * (self.outlets[Idx][0:2] - self.outlets[Idx][3:5]))
            ...
            if Idx not in self.skipOutlet:
                ...
                self.insert(pya.Path(arrayToPointList([channelPath[0], channelPath[0]]),
                                     widthChannel, 0.5*widthChannel, 0.5*widthChannel, True))

                self.insert(pya.Path(arrayToPointList([channelPath[-1], channelPath[-2]]),
                                     widthChannel, 0.5*widthChannel, 0.5*widthChannel, True))
    ...
    def makeSpacers(self, sizeShape = 0.0, multi=16):
        ...
        for scarray in separations:
            ...
            poly = SpacerMain.polygon() # convert to polygon and size inversely
            poly.size(-1 * sizeShape + self.SpacerInvScale * 30e3)
            self.TargetCell.shapes(self._ly.layer(self.LayerInfoSepMerge)).insert(poly)
            polyPlaceMirror = poly.transform(pya.Trans(pya.Trans.M0, -0.0, -0.0))

            self.TargetCell.shapes(self._ly.layer(self.LayerInfoSepMerge)).insert(polyPlaceMirror)
        self.mergeShapes(self.LayerInfoSepMerge)
        self.shapeBoolean(self.LayerInfoSepMerge, self.LayerInfo, self.LayerInfo)
    ...
    def bboxViaSep(self, gradientCoils, LayerInfo):
        ...
        for gradient in gradientCoils:
            for shape in gradient.TargetCell.shapes(self._ly.layer(LayerInfo)).each():
                if (shape.bbox().right > self.channelLeft) and (self.channelRight > shape.bbox().left):
                    self.bboxViaSepList.append(shape.bbox())

    def viaYGridClose(self):
        """
        returns the a list of index lists which corresponds to shapes which may interfere with the channel routing.
        ...
        idx_sorted = sorted(range(len(self.bboxViaSepList)), key=lambda n:(self.bboxViaSepList[n].center().y * -1))
        ...
        for idx in idx_sorted:
            ...
            for idxN in idx_sorted: # check if other via's interfere!
                ...
                if ((idxN != idx) and  # via is other than outer for
                    (self.bboxViaSepList[idxN].p2.y >= self.bboxViaSepList[idx].p1.y - self.viaExtraSep) and
                    (self.bboxViaSepList[idxN].p1.y <  self.bboxViaSepList[idx].p2.y + self.viaExtraSep)):
    ...
    def channelXPosFit(self, idxListVia):
        ...
        for n in sorted(range(len(inSectVias)), key=lambda n:(inSectVias[n].p1.x)):
        ...
        for Via in viaItvLst:
            ...
            if len(gapItvLst) == 0:
                ...
                if (Via[0] <= self.channelLeft) and (Via[1] > self.channelLeft):
        ...
        if len(gapItvLst) == 0:
            ...
            if (viaPre[1] < self.channelRight): # and viaPre[0] > gapItvLst[-1][1]):
    ...
    def fludicConnector(self, sizeShape = 0.0):
        ...
        points = np.array([[self.x1, self.y1], [self.x1 + 162.6, self.y1 - 162.6]]) * 1e3
        connectors = [points, points * [-1, 1], points * [1, -1], points * [-1, -1]]
        for con in connectors: # make the 4 fluidic connectors
            connectorPath = pya.Path( arrayToPointList(con),
                                      ...
                                      0.5 * (self.widthMain + 2 * sizeShape), True)
        ...
        for gap in gapItvLst:
            if gap[0] == self.channelLeft:
                if gap[1] == self.channelRight:
                    rtvXPos = [*rtvXPos, *self.arrangeC(gap[0], gap[1], addsep=addsep)]
                else:
                    rtvXPos = [*rtvXPos, *self.arrangeL(gap[0], gap[1] - self.resistMinWidth, addsep=addsep)]
            elif gap[1] == self.channelRight:
                rtvXPos = [*rtvXPos, *self.arrangeR(gap[0] + self.resistMinWidth, gap[1], addsep=addsep)]
            else: # channel in the middle
                rtvXPos = [*rtvXPos, *self.arrangeC(gap[0] + 1.5 * self.resistMinWidth, gap[1] - self.resistMinWidth, addsep=addsep)]
        return rtvXPos

    def getMaxChannels(self, viaIdxYClose):
        lstLen = []
        for viaYClose in viaIdxYClose: # viaYClose -- if via is in channel region
            lstLen.append ( len(self.channelPosFromItv( self.channelXPosFit(viaYClose))))
        if len(lstLen) == 0:
    ...
    def makeEqualChannelQuant(self, viaIdxYClose):
        ...
        for viaYClose in viaIdxYClose:
            addsep = 0.0
            while len(self.channelPosFromItv( self.channelXPosFit(viaYClose), addsep=addsep)) > maxChannel:
                addsep += 2.0 * 10**3
            addsepLst.append( addsep )
            if len(self.channelPosFromItv( self.channelXPosFit(viaYClose), addsep=addsep)) < maxChannel:
                repeat = True
    ...
    def makeChannelGrid(self, gradientCoils=[], LayerInfo=None, LayerInfoViaSep=None, sizeShape=0.0):
        ...
        for n in range(len(viaIdxYClose)):
            XchannelPos = self.channelPosFromItv(self.channelXPosFit(viaIdxYClose[n]),
                                                 addsep=addsep[n])
...
def makeFluidicCooling(cooling, TargetCellName, removeCell=True, gradientCoils=[]):
    ...
    :param gradientCoils:  to get the vias of the gradint coils. The return of the Gradient Coil creator
    """
    ...
    if cooling["@type"] == "comb":
        ...
        skip = np.append( b, (np.where(np.logical_and(a>=-1580e3, a <= -770e3))))
    ...
    for n in range(len(fc.LayerInfoFCroute)):
        ...
        if (fc.LayerInfoFCroute[n].layer in fc.LFchannels):
            if ((fc.LayerInfoFCroute[n].layer not in fc.LSkipPrimer)
                and not (fc.LayerInfoFCroute[n].layer == 4 and (fc.LayerInfoFCroute[n].datatype == 0))):
                fc.SkipPrimerBool = False
                fc.makeMainSuppylChannel(sizeShape=size)
            else:
                fc.SkipPrimerBool = True
        ...
        if fc.LayerInfoFCroute[n].layer in fc.LEncapsulation:
            ...
            if cooling["@type"] == "comb" and fc.LayerInfoFCroute[n].layer == 3:
