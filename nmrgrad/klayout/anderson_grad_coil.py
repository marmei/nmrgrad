...
class StraightGradCoil(KlayoutPyGradCoil):
    ...
    def wire_to_conductor(cond, z_not, mirror_chip=1.0, mirror_y=1.0):
        ...
        return np.asarray([cond[:, 0] * 1e-3 * mirror_chip,
                           ...
                           - mirror_chip * z_not * 1e-3]).swapaxes(0, 1)
    ...
    def makeInterconnect(self, connector, pos,
        ...
        self.TargetCell.insert(pya.CellInstArray.new(
            helperCell.cell_index(), trans))
        if connector == 0.0:
            vz = -1.0
        else:
            vz = 1.0

        array1 = np.array([[self.topChipWidth,
                            connector + .5 * vz * self.widthConnector],
                           [self.topChipWidth - self.lenConnector - addlength,
                            connector + .5 * vz * self.widthConnector],
                           [pos[0] - 0.43 * self.lenVia,
                               pos[1] + vz * self.yOffsetVia],
                           [pos[0] - 0.43 * self.lenVia,
                               pos[1] - vz * 0.43 * self.lenVia],
                           [pos[0] + 0.43 * self.lenVia,
                               pos[1] - vz * 0.43 * self.lenVia],
                           [pos[0] + 0.43 * self.lenVia, pos[1]],
                           [self.topChipWidth - self.lenConnector,
                               connector - .5 * vz * self.widthConnector],
                           [self.topChipWidth,
                            connector - .5 * vz * self.widthConnector]])
        ...
        if self.LayerViaInfosCoil is None:
    ...
    def make_anderson_z_grad(self):
        ...
        via01 = np.array([self.xPosCondStartZ,  z0 - self.viaYcorrectZ])
        self.makeVia(via01)
        self.makeVia(via01 * np.array([1, -1]))  # other via simmilar

        self.makeInterconnect(self.connector01, via01,
                              trans_angle=pya.Trans.M90)

        # mirror the one interconnect
        if self.InterConnectTop is True:
            self.makeInterconnect(self.connector01, via01,
                                  trans_angle=pya.Trans.R180)
        else:
        ...
        Zgrad_coil = np.asarray([via01,
                                 np.array([self.xPosCondStartZ -
                                           self.viaYcorrectZ * .07, z0]),
                                 np.array([-self.xPosCondStartZ +
                                           self.viaYcorrectZ * .07, z0]),
        ...
        self.wire_to_round_path(self.mirrorArrayXaxis(
            Zgrad_coil), radius=self.width * 20e-3)
        self.wire_to_round_path(self.mirrorArrayXaxis(
            Zgrad_coil2), radius=self.width * 1e-3)
        self.wire_to_round_path(self.mirrorArrayXaxis(
            Zgrad_coil3), radius=self.width * 10e-3)
        self.wire_to_round_path(self.mirrorArrayXaxis(
            Zgrad_coil4), radius=self.width * 10e-3)

        arc_intecon = placeArc(self.TargetCell, self._ly,
                               trans=pya.Trans(
                                   pya.Trans.R0, self.xPosCondStartZ, 0),
                               r1=(z0 - self.viaYcorrectZ - .5 *
                                   self.widthVia) / 1e3,
                               r2=(z0 - self.viaYcorrectZ + .5 *
                                   self.widthVia) / 1e3,
        ...
        intercon_02 = np.asarray([
                via01 * np.array([1.0, -1.0]),  # Zgrad_coil[0],
                [via01[0], 0],
                [4000e3, 0]]) * -1e-3
        ...
        coilZ = np.vstack([
            self.wire_to_conductor(
                intercon_01[::-1], self.z0 + 100, mirror_y=-1.0),
            ...
            self.wire_to_conductor(
                intercon_01[::-1], - self.z0 - 100),
            self.wire_to_conductor(
                Zgrad_coil[::-1], - self.z0, mirror_y=-1.0),
            self.wire_to_conductor(
                arc_intecon, -self.z0 - 100),
            self.wire_to_conductor(Zgrad_coil, -self.z0),
            self.wire_to_conductor(
                intercon_02, -self.z0 - 100),
        ...
        if self.zNOT_top_real is not None:
            ...
            coilZ = np.vstack([self.wire_to_conductor(intercon_01[::-1],
                                                      base_layer,
                                                      mirror_y=-1.0),
                               self.wire_to_conductor(
                                   Zgrad_coil[::-1], self.zNOT_top_real),
                               self.wire_to_conductor(
                                   Zgrad_coil, self.zNOT_top_real,
                                   mirror_y=-1.0),
                               self.wire_to_conductor(intercon_01, base_layer),
                               self.wire_to_conductor(
                                   intercon_01[::-1], - base_layer),
                               self.wire_to_conductor(
                                   Zgrad_coil[::-1], - self.zNOT_top_real,
                                   mirror_y=-1.0),
                               self.wire_to_conductor(
                                   Zgrad_coil, -self.zNOT_top_real),
                               self.wire_to_conductor(
                                   intercon_02, -base_layer),
                               self.wire_to_conductor(intercon_03,
                                                      base_layer)])

        self.conductors = [
            [coilZ, {"color": "b", "zorder": 210, "current": 1.0}]]
    ...
    def make_anderson_y_grad(self):
        ...
        GradYStr, CondY01, CondY02 = \
            GradY.max_grad_z_comb(self.z0 * 1e-3,
                                  separation=(self.resistMinWidth
                                              + self.width) * 1e-6) \
            * np.array([1, 1e6, 1e6])
        z0 = self.z0 * 1e3  # convert to nm
        ...
        self.makeInterconnect(self.connector02, via01,
                              trans_angle=pya.Trans.M90)

        # make the 1st conductor (outer or top)
        CondY02_Ypos_end = np.array(
            [self.xPosCondStartY - 1.5 * self.viaYcorrectZ, CondY02])
        ...
        CondY02_Ypos_02 = np.array([CondY02_Ypos_end
                                    + 45e3 * np.array([1, 0]),
                                    np.array([-self.xPosCondStartY
                                              + self.viaYcorrectZ,
                                              CondY02]) + thickerVia,
                                    via01 * np.array([-1, 1]) + thickerVia])
        r1_right = (CondY02 - (z0 - self.viaYcorrectZ) - .5 *
                    self.width - 25e3) / 1e3
        ...
        CondY02ToArch = np.array([CondY02_Ypos_end
                                  + np.array([0, self.width * .5]),
                                  CondY02_Ypos_end -
                                  np.array([0, self.width * .5]),
                                  np.array(
                                      [xArch_right, r1_right * 1e3
                                       + yArch_right]),
                                  np.array([xArch_right,
                                            r2_right * 1e3 + yArch_right])])
        ...
        ArchtoCondY01 = np.array([np.array([xArch_right,
                                            ...
                                            - self.widthArc * .5]),
                                  np.array([xArch_right
                                            - 0.75 * self.lenVia, yArch_right
                                            - r1_right * 1e3
                                            - self.widthArc * .5])])

        CondY01_Via = np.array([xArch_right - .75 * self.lenVia,
                                yArch_right - r1_right * 1e3
                                - self.widthArc * .5
                                + .5 * (self.width
                                        - self.widthArc)])  # start from via
        ...
        CondY01_top = np.array([CondY01_Via * np.array([1, 1]),
                                CondY01_Start * np.array([1, 1]),
                                CondY01_Start *
                                np.array([-1, 1]), CondY01_Via *
                                np.array([-1, 0])
                                + (self.xPosCondStartZ + .5 *
                                   self.lenVia - .5 * self.widthVia)
                                * np.array([0, 0])
                                + (CondY01_Via - self.width - 85e3)
                                * np.array([0, -1])])

        r_centre_outer = CondY01_top[3][1] - \
            yArch_right * -1 - self.width * 0.5

        # connector from the innerConductor CondY01 posY (1) lower to the
        # outerRing
        CondY01_pos_left_RingCon = np.array([np.array([-xArch_right
                                                       + 0.75 * self.lenVia,
                                                       - yArch_right
                                                       + r_centre_outer
                                                       + self.widthArc * .5]),
        ...
        arrayConArchInner = np.array([np.array([-xArch_right
                                                + .75 * self.lenVia,
                                                -1 * (yArch_right
                                                      - r1_right * 1e3
                                                      - self.widthArc * .5)]),
        ...
        via02 = np.array(
            [xArch_right + r2_right * 1e3 - 0.5 * self.dViaOld,
             CondY02 + 100e3])  # z0 + self.viaYcorrectZ + .5 * self.widthVia])

        self.makeInterconnect(self.connector02, via02,
                              trans_angle=pya.Trans.R180)
        ...
        moveItIntercon = arrayInnerVia02con[0][0] - \
            self.widthArc - self.resistMinWidth * 1.5
        OuterInterconVia02 = np.array([[moveItIntercon,
                                        arrayInnerVia02con[0][1]],
                                       [moveItIntercon,
                                        arrayInnerVia02con[1][1]]])

        VerticalConnectIntercon = np.array([[-via02[0]
                                             + (.5 * self.lenVia
                                                - .5 * self.widthVia),
                                             -via02[1]
                                             - r_outer_lowX * 1e3
        ...
        CondY02_Ypos_01_mirror = np.append(CondY02_Ypos_01_mirror,
                                           ...
                                           * np.array([-1.0, 1.0]), axis=0)

        CondY02_Ypos_01_mirror = np.append(CondY02_Ypos_01_mirror,
                                           np.array([[
                                               VerticalConnectIntercon[0][0]
                                               + r_outer_lowX * 1e3
                                               + self.width * .5,
                                               - via02[1]]]), axis=0)
        ...
        std_path_cond = np.vstack([CondY02_Ypos_02[::-1],
                                   ...
                                   # CondY01_pos_left_RingCon[::-1][0][1]],

                                   [CondY01_top[-1][0],
                                    CondY01_top[-1][1]],

                                   [OuterInterconVia02[0][0] - 10 * \
                                       10**3,
                                       CondY01_top[-1][1]],
                                   ...
                                   - np.array([10,
                                               r_outer_lowX + .5
                                               *
                                            self.widthArc * 1e-3, ]) * 1e3,

                                   [[CondY02_Ypos_01_mirror[::-1][0][0],
                                     OuterInterconVia02[1][1]
                                     - r_outer_lowX * 1e3
                                     - .5 * self.widthArc]],

                                   CondY02_Ypos_01_mirror[::-1],
                                   [[xarchRight + 100e3, -1.0 * \
                                       CondY02_Ypos_02[::-1][-1][1]]],
                                   ...
                                   [[arrayInnerVia02con[0][0],
                                       arrayConArchInner[-1][1]]],
        ...
        add_path_cond = np.vstack([CondY02_Ypos_01[::-1],
                                   ...
                                   [CondY01_top[-1][0], CondY01_top[-1][1]],

                                   [OuterInterconVia02[0][0]
                                    - 80e3, CondY01_top[-1][1]],

                                   shrExt(OuterInterconVia02
                                          - np.array(
                                                  [0,
                                                   r_outer_lowX
                                                   + .5 * self.widthArc * 1e-3
                                                   + 40]) * 1e3,
                                          value=80e3),
                                   ...
                                   [[CondY02_Ypos_01_mirror[::-1][0][0],
                                     OuterInterconVia02[1][1]
                                     - r_outer_lowX * 1e3
                                     - .5 * self.widthArc - 40e3]],
                                   shrExt(
                                       CondY02_Ypos_01_mirror[::-1],
                                       -100e3),

                                   [[xarchRight + 200e3, -1.0 * \
                                       CondY02_Ypos_02[::-1][-1][1]]],

                                   [[xarchRight + 200e3,
                                     -1.0 * CondY01_top[0][1]]],

                                   shrExt(self.mirrorArrayXaxis(
                                       CondY01_top)[:-1], -140e3),
        ...
        if self.zNOT_top_real is None:
            self.conductors.append([self.wire_to_conductor(Ygrad_coil,
                                                           self.zNOT_top_real),
                                    {"color": "b", "zorder": 210,
                                     "current": -1.0}])
            self.conductors.append([self.wire_to_conductor(Ygrad_coil,
