...
class KlayoutPyGradCoil(KlayoutPyFeature):
    ...
    def __init__(self, TopCellName, GradCoil):
        ...
        if "@zNOTCal_bot" in GradCoil:
            ...
            self.ccSep01  = eval(GradCoil["@zNOTCal_bot"]) # coil centre separation 01
            self.zNOTCal_bot = eval(GradCoil["@zNOTCal_bot"])

        if "@zNOTCal_top" in GradCoil:
            self.zNOT_top = eval(GradCoil["@zNOTCal_top"])
            self.ccSep00  = eval(GradCoil["@zNOTCal_top"]) # coil centre separation 00
    ...
    def makeCoilTrack(self,
        ...
        if self._ly.has_cell("helperCellAmark") == True:
    ...
    def InterConPts(self, cv):
        ...
        if len(cv) == 2:
            cv = np.asarray(cv)
            interSecPts  = np.array([np.linspace(cv[0][0], cv[1][0],
    ...
    def GapsIntoCoilList(self, gapList, ptsIntercon,
        ...
        curve but must intersect with the gradient coil in oder to create the gaps.
        """
        ...
        for coilIdx in range(len(gapList)):
            ...
            if coil.shape[0] >= 80 and min_dist_array_at_pts(coil, ptsIntercon) < 50e3:
                if skipFirst is True:
                    skipFirst = False
                    coilArray = array_idx_dist(centre_array_at_pts(coil, ptsIntercon),
                                                   distMax=distMaxIcon)
                elif skipLast == True and coilIdx == len(gapList)-1:
                    coilArray = array_idx_dist(centre_array_at_pts(coil, ptsIntercon),
    ...
    def makeSingleCurve(self, listOfGaps, ptsIntercon, shift=False,
        ...
        if (shift is True) and (cIntercon != []):
            ...
            if xDist == 0.0:
                ...
                if (xDist < 0) and (yDist < 0):
                    ...
                    signY =  1.0
                    signX = -1.0
                shiftStartEnd = np.array([np.sin(anglTan) * signX,
        ...
        ptsInterconX_end = np.swapaxes(ptsIntercon, 0,1)[0] - shiftStartEnd[0] * 2.0
        ptsInterconY_end = np.swapaxes(ptsIntercon, 0,1)[1] - shiftStartEnd[1] * 2.0

        ptsIntercon_end = np.swapaxes(np.array([ptsInterconX_end, ptsInterconY_end]), 0,1)
        ...
        for coilIdx in range(len(listOfGaps)):
            if coilIdx == 0 and type(startPts) == bool: # Start
                if startPts is True:
                    ...
                    spline = np.append(ptsIntercon[0:2], listOfGaps[coilIdx][:2], axis=0)
                    new_array = np.append(bspline(spline), listOfGaps[coilIdx][3:-2], axis=0)
            elif coilIdx == len(listOfGaps)-1: # End
                spline = np.append(listOfGaps[coilIdx-1][-2:], listOfGaps[coilIdx][:2], axis=0)
                new_array = np.append(new_array, bspline(spline), axis=0)
                new_array = np.append(new_array, listOfGaps[coilIdx][3:-2], axis=0)
                if connectEndPts is True:
                    spline    = np.append(listOfGaps[coilIdx][-2:], ptsIntercon_end[-2:], axis=0)
                    new_array = np.append(new_array, bspline(spline), axis=0)
            else: # inbetween
                if type(startPts) == bool:
                    ...
                    spline = np.append(startPts, listOfGaps[coilIdx][:2], axis=0)
                    new_array = bspline(spline)
                    startPts = False
                new_array = np.append(new_array, listOfGaps[coilIdx][3:-2], axis=0) # ref before
        return new_array
    ...
    def export_conductor_model(self, addName=""):
        """
        Export the coil path.

        """
        fname_out = self.dir_grad_output + "CModel_" + self.TopCell.name + "_" \
    ...
    for w_idx, w_elem in enumerate(wire):
        ...
        if len_pt_to_pt > len_max:
            x_step = np.linspace(w_elem[0], wire[w_idx+1][0],
                                 int(len_pt_to_pt / len_max + 1), endpoint=False)
